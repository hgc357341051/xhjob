...
- 新增 Docsify 入口：`docs/index.html`（引入 docsify.js + 插件，配置侧边栏/封面/搜索/代码高亮）
- 新增 `docs/_sidebar.md`（Docsify 左侧导航，6 分组层级）
- 新增 `docs/_coverpage.md`（封面页：项目名 + 简介 + 快速入口）
- 新增 `docs/.nojekyll`（告知 GitHub Pages 静态模式不忽略 `_` 开头文件）
- 新增 `docs/README.md`（Docsify 默认首页内容，替代原 `index.md`）
