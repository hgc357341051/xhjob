...
> 基于 Rust（ext-php-rs 0.15）内核的 PHP 扩展，为 ThinkPHP 8 提供**单机异步任务调度**能力，零外部依赖（不依赖 Redis / RabbitMQ / Swoole / Supervisor），支持**两种任务执行池模式**（多协程池 / 多线程池）按场景切换。
...
- **两种任务执行池模式**（重点能力）
  - 多协程池（`coroutine`，默认）：基于 tokio async runtime，IO 密集型场景高并发、低资源占用，最大并发 1024。
  - 多线程池（`thread`）：基于 `std::thread` + `crossbeam-channel`，每个任务在独立工作线程中 `block_on` 执行，适合 CPU 密集型或需严格并发控制的场景。
...
### 模式一：多协程池（coroutine，默认）

- **底层实现**：基于 `tokio` 多线程 async runtime，任务以 future 形式提交到共享的协程调度器，由 tokio 工作线程复用执行。
- **并发模型**：M:N 调度——大量任务复用少量 OS 线程，每个任务在 `.await` 让出点主动切换，不阻塞工作线程。
- **资源占用**：单任务协程栈按需增长（典型几 KB），内存占用低；线程数固定，不随任务数线性增长。
- **最大并发**：默认 `1024`，可通过 `XHJOB_COROUTINE_POOL_SIZE` 环境变量覆盖（向上调优需关注文件描述符上限）。
- **适用场景**：
  - IO 密集型任务：HTTP 请求、shell 命令等待、数据库 / 缓存往返。
  - 高并发短任务：定时轮询、心跳上报、批量 webhook 回调。
  - 资源敏感环境：容器、低配 VPS、需与 PHP-FPM 共享内存的部署。
- **启用方式**：`XHJOB_POOL_MODE=coroutine`（或不设置，默认值）。
...
| 维度 | 多协程池（coroutine，默认） | 多线程池（thread） |
|---|---|---|
| 底层实现 | tokio async runtime | std::thread + crossbeam-channel |
| 调度模型 | M:N（多任务复用少线程） | 1:1（一线程一任务） |
| 默认并发上限 | 1024（`XHJOB_COROUTINE_POOL_SIZE`） | CPU 核数（`XHJOB_THREAD_POOL_SIZE`） |
| 单任务内存开销 | 几 KB（协程栈按需增长） | 2~8 MB（OS 线程栈） |
...
│   │  date)      │   │  coalesce)  │   │  coroutine | thread │   │
│   └─────────────┘   └─────────────┘   └─────────┬───────────┘   │
│                                                  │               │
│              ┌───────────────────────────────────┴──────────┐    │
│              ▼                                              ▼    │
│   ┌──────────────────────────┐              ┌────────────────────┐│
│   │ coroutine 模式           │              │ thread 模式        ││
│   │ ┌──────────────────────┐ │              │ ┌────────────────┐ ││
...
```bash
# 多协程池（默认），自定义并发上限
export XHJOB_POOL_MODE=coroutine
export XHJOB_COROUTINE_POOL_SIZE=2048
...
return [
    ...
    'pool_mode'    => env('XHJOB_POOL_MODE', 'coroutine'), // 'coroutine' | 'thread'
];
```
...
```bash
export XHJOB_POOL_MODE=coroutine   # 或 thread
...
| `pool_mode` | string | `'coroutine'`（`XHJOB_POOL_MODE`） | 任务执行池模式：`'coroutine'`（多协程池）或 `'thread'`（多线程池）。 |
...
| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `XHJOB_POOL_MODE` | `coroutine` | 任务执行池模式：`coroutine`（多协程池）或 `thread`（多线程池）。 |
| `XHJOB_COROUTINE_POOL_SIZE` | `1024` | 多协程池模式下的最大并发任务数。 |
...
| `test_xhjob_pool_mode.php` | — | 两种池模式对比测试：coroutine 与 thread 模式下的吞吐、并发、资源占用对比。 |
...
# 4. 池模式对比测试
XHJOB_POOL_MODE=coroutine php -d extension=$EXT test_xhjob_pool_mode.php
