...
// | 证明 async（tokio async）与 thread（std::thread）是两种本质不同的并发模型。
// |
// | 验证维度：
// |   1. 线程名：async 模式有 xhjob-tokio，thread 模式有 xhjob-worker-N
// |   2. 并发行为：async 模式 4 任务全部同时执行；thread 模式(pool=2) 分 2 批
// |   3. 总耗时：async ≈ 1×sleep；thread ≈ 2×sleep（分批排队）
// +----------------------------------------------------------------------
...
step(3, '线程名验证：xhjob-tokio 存在，xhjob-worker 不存在', function () use ($SERVICE, $DATA_DIR) {
    ...
    assertTrue(count($counts['tokio']) > 0, 'async模式应存在 xhjob-tokio 线程');
    assertTrue(count($counts['workers']) === 0,
        'async模式不应存在 xhjob-worker 线程，实际有 ' . count($counts['workers']));
    echo "（tokio=" . count($counts['tokio']) . ', workers=0）';
});

step(4, "并发测试：{$TASK_COUNT} 个 sleep({$SLEEP_SEC}) 时间戳分布", function () use ($mgr, $MARKER_DIR, $TASK_COUNT, $SLEEP_SEC, &$asyncResult) {
    ...
    assertTrue($earlyCnt === $TASK_COUNT,
        "async模式应全部 {$TASK_COUNT} 个任务在 1s 内开始，实际 {$earlyCnt}");
...
$corDelta = $asyncResult
    ? round($asyncResult['deltas'][0]) . '~' . round(end($asyncResult['deltas'])) . 'ms'
    : 'N/A';
$thrDelta = $threadResult
    ? round($threadResult['deltas'][0]) . '~' . round(end($threadResult['deltas'])) . 'ms'
    : 'N/A';
$corTotal = $asyncResult ? round($asyncResult['total_ms'] / 1000, 2) . 's' : 'N/A';
...
$line('维度', 'async模式(async)', '线程模式(thread,pool=2)');
echo str_repeat('-', 78) . "\n";
...
echo "\n结论：\n";
echo "  async模式：任务在 tokio async runtime 上调度，通过 Semaphore 控制并发。\n";
